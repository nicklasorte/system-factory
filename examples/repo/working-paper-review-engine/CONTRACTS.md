...
This repository follows the canonical artifact contracts defined in `nicklasorte/spectrum-systems` (governed by `nicklasorte/spectrum-systems`). It must not redefine or fork those contracts locally. Instead, it declares how it consumes and/or produces artifacts under those contracts and escalates changes upstream first.

What this repo does:
- Adopts canonical contracts from `nicklasorte/spectrum-systems` (see `config/contracts.yaml`).
...
- `contracts/` contains integration notes and placeholders for adapters pointing at `nicklasorte/spectrum-systems`.

Alignment tasks:
- Keep `standards_source_repo` pinned to `nicklasorte/spectrum-systems` and track `standards_source_version` changes.
